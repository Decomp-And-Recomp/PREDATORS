...
public class SoldierAutoRifle : BaseEnemy
{
	...
	public float berserkFireRate = 0.3f;
	...
	public LineRenderer flashLightLineRenderer;
	...
	{
		if (playerController.JungleType == 2 || playerController.JungleType == 4)
		{
			if ((bool)flashLightLineRenderer)
			{
				flashLightLineRenderer.enabled = true;
			}
		}
		else if ((bool)flashLightLineRenderer)
		{
			Object.Destroy(flashLightLineRenderer.gameObject);
		}
		if ((bool)muzzleFlash)
		{
	...
	protected override void EnemyDied()
	{
		if ((bool)flashLightLineRenderer)
		{
			flashLightLineRenderer.enabled = false;
		}
		if ((bool)muzzleFlash)
		{
			muzzleFlash.enabled = false;
	...
	{
		RemoveSelfFromArray();
		survivalMissionController.EnemySoldierRifleDied(index, deathType);
	}
