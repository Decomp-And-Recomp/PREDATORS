...
public class Cuchillo : BaseEnemy
{
	...
	private Transform particleXform;

	private Transform muzzleRXform;

	private Transform muzzleLXform;

	private void Start()
	{
		if ((bool)particleBulletsEmitter)
		{
			particleXform = particleBulletsEmitter.transform;
		}
		else
		{
			Debug.LogError("particle bullets emitter not set on cuchillo");
		}
		if ((bool)muzzleFlash)
		{
			muzzleRXform = muzzleFlash.transform;
		}
		else
		{
			Debug.LogError("muzzle R not set on cuchillo");
		}
		if ((bool)muzzleFlash2)
		{
			muzzleLXform = muzzleFlash2.transform;
		}
		else
		{
			Debug.LogError("muzzle L not set on cuchillo");
		}
	}
	...
	protected override IEnumerator Berserk()
	{
		...
		while (!isDead)
		{
			...
			if (shootTimer > 0.5f)
			{
				...
				{
					particleXform.position = muzzleRXform.position;
					particleBulletsEmitter.Emit(1);
					particleXform.position = muzzleLXform.position;
					particleBulletsEmitter.Emit(1);
	...
	protected override IEnumerator Attack()
	{
		...
		float blockTimer = 0f;
		float squaredAttackShootDistance = attackShootDistance * attackShootDistance;
		while (!isDead)
		{
			blockTimer += Time.deltaTime;
			...
			if (predatorTargetSquareDistance < attackDistance * attackDistance)
			{
				...
				if (predatorTargetSquareDistance < squaredAttackShootDistance && canShoot)
				{
	...
	{
		RemoveSelfFromArray();
		survivalMissionController.EnemyCuchilloDied(index, deathType);
	}
